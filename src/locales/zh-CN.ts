export default {
  ...
  app: {
    ...
    freeThreadedGilFallbackWarning:
      'free-threaded 运行时已退化为 GIL 模式，请在插件页查看标记为「GIL 回退」的插件或检查后端日志中的原生扩展兼容告警',
  },
  pwa: {
  ...
  githubToken: {
    title: 'GitHub Token',
    initializationTitle: '设置 GitHub Token（可选）',
    initializationSection: 'GitHub API 访问',
    description:
      'Agent 用此 Token 提交 Issue/PR。Device Flow 会请求公开、私有仓库读写及工作流更新权限；Token 仅保存在服务端。',
    notConfigured: '尚未设置 GitHub Token。授权或填写 PAT 后，Agent 可提交 Issue/PR。',
    connected: '已连接 GitHub',
    statusChecking: '检测中',
    statusNotConfigured: '未连接',
    statusConnected: '已连接',
    statusNeedsReauthorization: '需重新授权',
    connect: '一键授权 GitHub',
    reauthorize: '重新授权',
    disconnect: '解除授权',
    needsReauthorization: '当前授权已失效、即将过期或缺少 Agent 提交 Issue/PR 所需权限，请重新授权。',
    manualTitle: '手动填写或更换 Token',
    manualHint:
      '保存手动 PAT 会替换当前 GitHub Token。PAT 只保存在服务端；请确保有目标仓库所需的 Issue、Fork、分支推送和 PR 权限；修改工作流文件还需 workflow 权限。',
    manualLabel: 'GitHub PAT',
    manualPlaceholder: 'ghp_**** 或 github_pat_****',
    saveManual: '保存 Token',
    saved: 'GitHub Token 已保存',
    disconnected: 'GitHub Token 已清除',
    authTitle: '授权 GitHub',
    authSubtitle: 'GitHub 会展示本次请求的权限。确认后，MoviePilot 会在服务端保存授权，供 Agent 提交 Issue 和 PR。',
    authInstructions: '打开 GitHub 验证页面，登录后输入下面的用户码：',
    userCode: '用户码',
    copyUserCode: '复制用户码',
    copied: '已复制',
    openGithub: '打开 GitHub 验证页面',
    openLink: '点击这里打开',
    popupBlocked: '浏览器拦截了新窗口，请使用下面的链接打开 GitHub。',
    waiting: '正在等待 GitHub 授权结果…',
    clickPoll: '完成授权后点击检查',
    checkAuthorization: '检查授权状态',
    authPreparing: '正在准备 GitHub 授权…',
    authorized: 'GitHub 授权完成',
    promptTitle: '设置 GitHub Token',
    promptDescription: '设置后 Agent 可读取服务端 Token 提交 Issue/PR，也会提高 GitHub API 限流额度。',
    promptAction: '去设置',
    dismissPrompt: '永久关闭提示',
  },
  login: {
    wallpapers: '壁纸',
    tagline: '你的智能影视媒体库',
  ...
  setting: {
    ...
    system: {
      ...
      githubTokenHint: '用于 GitHub API 访问和 Agent 自动提交 Issue/PR；Token 需具备目标仓库权限。',
      ocrHost: '验证码识别服务器',
      ...
      aiAgentStatusDisabled: '未启用',
      aiAgentStatusEnabled: '已启用',
      llmProvider: 'LLM提供商',
      ...
      bangumiProxyEnableHint: '启用后，Bangumi 数据请求使用代理地址，图片请求通过后端图片代理端点访问',
      bangumiApiDomain: 'Bangumi 数据代理地址',
      ...
      bangumiImageDomain: 'Bangumi 图片代理地址',
      bangumiImageDomainPlaceholder: 'https://proxy.example/ 或 https://proxy.example/?url=',
      bangumiImageDomainHint: '留空使用原始图片地址；以 / 结尾替换主机，含 ? 按查询参数拼接，否则按路径拼接',
  ...
  plugin: {
    ...
    gilFallbackBadgeHint:
      '加载该插件后 free-threaded 运行时（v3t）已回退到 GIL 模式，失去多线程并行；停用该插件并重启可恢复，或切回标准 V3 镜像',
    ...
    incompatibleRuntime: '插件声明不支持 free-threaded 运行时（v3t），无法安装',
    ...
    suffix: '分身后缀（可选）',
    suffixAutoPlaceholder: '留空则自动分配，也可自定义，例如：Test、Backup',
    suffixHint: '用于区分分身的唯一标识，只能包含英文字母和数字，最长 20 个字符',
    suffixFormatError: '只能包含英文字母和数字',
    suffixLengthError: '长度不能超过20个字符',
    suffixAutoHint: '留空提交即请求服务端自动分配一个未被占用的后缀。',
    suffixAutoNumbering:
      '号段从 2 起（本体算第 1 个）；已装插件、已停用的分身、磁盘上留存的插件包目录都算占号，所以号码可能跳。',
    suffixAutoNeverRestores: '自动分配不会挑中下面清单里的任何一个号：不主动点恢复，就不会凭空继承到别人的配置。',
    suffixServerAuthority: '这里的校验只是即时提醒；后缀能否使用以服务端为准，例如这个号已被占用。',
    suffixMatchesRestorable: '这个后缀名下有一个已停用的分身「{name}」：提交即是恢复它，而不是新建。',
    cloneName: '分身名称',
    ...
    cloneRestorableEmpty: '该插件没有已停用的分身，当前只能新建。',
    cloneRestorableLoadFailed: '可恢复分身清单读取失败：仍可新建，但判断不出填写的后缀名下是否留有旧配置。',
    cloneRestorableHasConfig: '留有业务参数',
    cloneRestorableNoConfig: '无参数留存',
    cloneRestorePrevious: '沿用停用前留存的业务参数',
    cloneRestorePreviousOn: '它停用前配好的参数原样回来，接着用。',
    cloneRestorePreviousOff: '丢弃留存的旧配置，按插件模板重建一份空的。',
    cloneDisplayBlankCreate: '名称、描述、图标留空即为空。',
    cloneDisplayBlankRestore:
      '名称、描述、图标留空表示沿用这个分身停用前登记的那份；填了就以填的为准，会覆盖掉原来的。',
    cloneOfCloneNotice: '这是一个分身，分身不能再建分身。请回到源插件 {id} 上创建。',
    cloneOfCloneNoticeGeneral: '这是一个分身，分身不能再建分身。请回到它的源插件上创建。',
    createClone: '创建分身',
    restoreClone: '恢复分身',
    cloning: '正在创建 {name} 的分身...',
    cloneRestoring: '正在恢复 {name} 的分身...',
    cloneSuccess: '插件分身 {name} 创建成功！',
    cloneRestoreSuccess: '分身 {name} 已恢复，停用前的配置继续可用',
  ...
  initialization: {
    ...
    usernameLockedHint: '已由配置文件或环境变量固定，不能修改',
    password: '设置密码',
