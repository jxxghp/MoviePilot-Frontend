export default {
  ...
  setting: {
    ...
    directory: {
      ...
      directorySaveOutdated: '保存期间目录已被继续修改，请再次保存最新草稿',
      ...
      matchModeHints: {
        sequential: '保持现有行为，使用第一个满足条件的目录。',
        specificity: '优先选择精确类别目录，其次是媒体类型目录和通配目录。',
      },
      routePreview: {
        title: '路由预览',
        subtitle: '使用当前未保存的分类与目录配置检查整理路径。',
        movie: '电影',
        tv: '电视剧',
        mediaType: '媒体类型',
        mediaTitle: '标题',
        year: '年份',
        providedCategory: '指定类别',
        genreIds: '类型 ID',
        ...
        sourcePath: '源路径',
        targetStorage: '目标存储',
        destinationPath: '指定目标路径',
        includeUnsorted: '包含未启用监控整理的目录',
        run: '开始预览',
        requestFailed: '路由预览失败：{message}',
        empty: '填写媒体元数据后开始预览。',
        result: '当前模式结果',
        noRoute: '没有目录满足当前条件。',
        noRouteShort: '无匹配目录',
        ...
        diagnostics: '配置诊断',
        categoryRules: '分类规则',
        noCategoryRules: '当前媒体类型没有分类规则。',
        directoryCandidates: '目录候选',
        noCandidates: '没有可展示的目录候选。',
        matched: '命中',
        notMatched: '未命中',
        selected: '已选择',
        missingValue: '缺失',
        ...
        warnings: {
          unconditional_category_not_last: '无条件兜底分类不是最后一项，后续规则不可达。',
          multiple_category_matches: '同时命中多条分类规则，实际采用第一条。',
          generic_before_specific: '通用目录会先于后面的精确目录命中。',
          unknown_media_category: '目录引用了当前分类配置中不存在的类别。',
          duplicate_directory_conditions: '存在条件相同但目标不同的目录配置。',
          provided_category_conflict: '指定类别与自动分类结果不同，当前使用指定类别。',
          no_matching_directory: '没有目录满足当前媒体与路径约束。',
        },
        reasons: {
          source_path_scope_mismatch: '源路径不在该目录的监控范围内。',
          monitor_disabled: '该目录未启用监控。',
          source_storage_mismatch: '源存储不匹配。',
          target_storage_mismatch: '目标存储不匹配。',
          destination_path_mismatch: '指定目标路径不属于该目录。',
          media_rule_mismatch: '媒体类型或类别条件不匹配。',
        },
      },
    },
    category: {
      ...
      saveOutdated: '保存期间配置已发生变化，已保留最新草稿，请再次保存。',
      loadFailed: '加载分类配置失败',
