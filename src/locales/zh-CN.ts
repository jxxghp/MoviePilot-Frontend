export default {
  ...
  app: {
    ...
    freeThreadedGilFallbackWarning: 'free-threaded 运行时已退化为 GIL 模式，请在插件页查看标记为「GIL 回退」的插件或检查后端日志中的原生扩展兼容告警',
  },
  pwa: {
  ...
  githubToken: {
    ...
    description: '用于提高 GitHub API 限流额度，让插件、资源和更新访问更稳定。',
    notConfigured: '尚未设置 GitHub Token，可以使用 GitHub 授权或手动填写 PAT。',
    ...
    reauthorize: '重新授权',
    disconnect: '解除授权',
    needsReauthorization: 'GitHub Token 已失效或即将过期，请重新授权。',
    manualTitle: '手动填写 Token',
    manualHint: '如果无法使用一键授权，也可以粘贴 GitHub PAT；Token 只会保存在服务端。',
    ...
    authTitle: '授权 GitHub',
    authSubtitle: '在 GitHub 页面确认授权后，MoviePilot 会自动完成设置。',
    ...
    promptDescription: '设置后可提高 GitHub API 限流额度，让插件和资源访问更稳定。',
    promptAction: '去设置',
  ...
  plugin: {
    ...
    gilFallbackBadgeHint: '加载该插件后 free-threaded 运行时（v3t）已回退到 GIL 模式，失去多线程并行；停用该插件并重启可恢复，或切回标准 V3 镜像',
