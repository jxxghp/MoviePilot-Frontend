export default {
  ...
  app: {
    ...
    freeThreadedGilFallbackWarning:
      'free-threaded 運行環境已退化為 GIL 模式，請在插件頁查看標記為「GIL 回退」的插件或檢查後端日誌中的原生擴展兼容警告',
  },
  pwa: {
  ...
  githubToken: {
    title: 'GitHub Token',
    initializationTitle: '設定 GitHub Token（可選）',
    initializationSection: 'GitHub API 存取',
    description:
      'Agent 使用此 Token 提交 Issue/PR。Device Flow 會請求公開、私人儲存庫讀寫及工作流程更新權限；Token 僅保存在伺服器。',
    notConfigured: '尚未設定 GitHub Token。授權或填寫 PAT 後，Agent 可提交 Issue/PR。',
    connected: '已連接 GitHub',
    statusChecking: '檢查中',
    statusNotConfigured: '未連接',
    statusConnected: '已連接',
    statusNeedsReauthorization: '需重新授權',
    connect: '一鍵授權 GitHub',
    reauthorize: '重新授權',
    disconnect: '解除授權',
    ...
    saveManual: '儲存 Token',
    saved: 'GitHub Token 已儲存',
    disconnected: 'GitHub Token 已清除',
    authTitle: '授權 GitHub',
    authSubtitle: 'GitHub 會顯示本次請求的權限。確認後，MoviePilot 會在伺服器保存授權，供 Agent 提交 Issue 和 PR。',
    authInstructions: '開啟 GitHub 驗證頁面，登入後輸入下面的使用者碼：',
    userCode: '使用者碼',
    copyUserCode: '複製使用者碼',
    copied: '已複製',
    openGithub: '開啟 GitHub 驗證頁面',
    openLink: '點擊這裡開啟',
    popupBlocked: '瀏覽器攔截了新視窗，請使用下面的連結開啟 GitHub。',
    waiting: '正在等待 GitHub 授權結果…',
    clickPoll: '完成授權後點擊檢查',
    checkAuthorization: '檢查授權狀態',
    authPreparing: '正在準備 GitHub 授權…',
    authorized: 'GitHub 授權完成',
    promptTitle: '設定 GitHub Token',
    promptDescription: '設定後 Agent 可讀取伺服器端 Token 提交 Issue/PR，也會提高 GitHub API 限流額度。',
    promptAction: '前往設定',
    dismissPrompt: '永久關閉提示',
  },
  login: {
    wallpapers: '壁紙',
    tagline: '你的智能影視媒體庫',
  ...
  notification: {
    ...
    webpush: {
      ...
      retry: '重新登記瀏覽器通知',
      permissionAllowed: '瀏覽器已允許通知，可重新登記 WebPush 訂閱。',
  ...
  subscribe: {
    ...
    musicSubscribeProgressTooltip: '已下載 {downloaded} · 共 {total} 首',
    completed: '訂閱完成',
    subscribing: '訂閱中',
    notStarted: '未開始',
    pending: '待定',
  ...
  setting: {
    ...
    system: {
      ...
      githubTokenHint: '用於 GitHub API 存取和 Agent 自動提交 Issue/PR；Token 需具備目標儲存庫權限。',
      ocrHost: '驗證碼識別服務器',
      ...
      aiAgentStatusDisabled: '未啟用',
      aiAgentStatusEnabled: '已啟用',
      llmProvider: 'LLM提供商',
      ...
      bangumiProxyEnable: 'Bangumi代理',
      bangumiProxyEnableHint: '啟用後，Bangumi 數據請求使用代理地址，圖片請求通過後端圖片代理端點訪問',
      bangumiApiDomain: 'Bangumi 數據代理地址',
      bangumiApiDomainPlaceholder: 'https://bangumi-proxy.example',
      bangumiApiDomainHint: 'Bangumi API 代理 Base URL，留空使用官方地址',
      ...
      bangumiImageDomain: 'Bangumi 圖片代理地址',
      bangumiImageDomainPlaceholder: 'https://proxy.example/ 或 https://proxy.example/?url=',
      bangumiImageDomainHint: '留空使用原始圖片地址；以 / 結尾替換主機，含 ? 按查詢參數拼接，否則按路徑拼接',
  ...
  plugin: {
    ...
    gilFallbackBadgeHint:
      '載入此插件後 free-threaded 運行環境（v3t）已回退到 GIL 模式，失去多執行緒並行；停用此插件並重啟可恢復，或切回標準 V3 映像',
    ...
    suffix: '分身後綴（可選）',
    suffixAutoPlaceholder: '留空則自動分配，也可自定義，例如：Test、Backup',
    suffixHint: '用於區分分身的唯一標識，只能包含英文字母和數字，最長 20 個字符',
    suffixFormatError: '只能包含英文字母和數字',
    suffixLengthError: '長度不能超過20個字符',
    suffixAutoHint: '留空提交即請求服務端自動分配一個未被佔用的後綴。',
    suffixAutoNumbering:
      '號段從 2 起（本體算第 1 個）；已裝插件、已停用的分身、磁碟上留存的插件包目錄都算佔號，所以號碼可能跳。',
    suffixAutoNeverRestores: '自動分配不會挑中下面清單裡的任何一個號：不主動點恢復，就不會憑空繼承到別人的配置。',
    suffixServerAuthority: '這裡的校驗只是即時提醒；後綴能否使用以服務端為準，例如這個號已被佔用。',
    suffixMatchesRestorable: '這個後綴名下有一個已停用的分身「{name}」：提交即是恢復它，而不是新建。',
    cloneName: '分身名稱',
    ...
    cloneRestoreTitle: '恢復插件分身',
    cloneRestoreNotice:
      '恢復出來的分身同樣默認為禁用狀態，需要手動啟用。它用的還是原來那個實例 ID，是同一個分身，不是新建的一個。',
    cloneRestorableTitle: '可恢復的已停用分身',
    cloneRestorableOnlyDisabled: '這裡只列已停用的分身；啟用中的分身配置正被使用，恢復它沒有意義。',
    cloneRestorableSameEndpoint: '恢復和新建是同一個入口：點一行即把它的後綴填進上面的輸入框，提交就是恢復那一行。',
    cloneRestorableEmpty: '該插件沒有已停用的分身，當前只能新建。',
    cloneRestorableLoadFailed: '可恢復分身清單讀取失敗：仍可新建，但判斷不出填寫的後綴名下是否留有舊配置。',
    cloneRestorableHasConfig: '留有業務參數',
    cloneRestorableNoConfig: '無參數留存',
    cloneRestorePrevious: '沿用停用前留存的業務參數',
    cloneRestorePreviousOn: '它停用前配好的參數原樣回來，接著用。',
    cloneRestorePreviousOff: '丟棄留存的舊配置，按插件模板重建一份空的。',
    cloneDisplayBlankCreate: '名稱、描述、圖標留空即為空。',
    cloneDisplayBlankRestore:
      '名稱、描述、圖標留空表示沿用這個分身停用前登記的那份；填了就以填的為準，會覆蓋掉原來的。',
    cloneOfCloneNotice: '這是一個分身，分身不能再建分身。請回到源插件 {id} 上創建。',
    cloneOfCloneNoticeGeneral: '這是一個分身，分身不能再建分身。請回到它的源插件上創建。',
    createClone: '創建分身',
    restoreClone: '恢復分身',
    cloning: '正在創建 {name} 的分身...',
    cloneRestoring: '正在恢復 {name} 的分身...',
    cloneSuccess: '插件分身 {name} 創建成功！',
    cloneRestoreSuccess: '分身 {name} 已恢復，停用前的配置繼續可用',
  ...
  initialization: {
    ...
    usernameLockedHint: '已由設定檔或環境變數固定，不能修改',
    password: '設定密碼',
