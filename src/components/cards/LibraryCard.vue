...
function getImgUrl(url: string) {
  if (!url)
    ...
    return `${import.meta.env.VITE_API_BASE_URL}system/img/0?imgurl=${encodeURIComponent(url).replace(/%2F/g, '/')}`
}
...
async function drawImages(imageList: string[]) {
  ...
  for (let i = 0; i < IMAGES.length; i++)
    IMAGES[i] = `${import.meta.env.VITE_API_BASE_URL}system/img/0?imgurl=${encodeURIComponent(IMAGES[i]).replace(/%2F/g, '/')}`
