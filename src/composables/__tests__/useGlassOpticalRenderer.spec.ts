...
describe('glass optical surface discovery', () => {
  ...
  it('keeps existing resize registrations while surfaces join, move and leave during a delivery', async () => {
    const three = await import('three')
    const root = document.createElement('div')
    const canvas = document.createElement('canvas')
    root.append(canvas)
    ...
    const added = appendOpticalSurface('app-hover-lift-card', { height: 180, width: 280, x: 400, y: 80 })

    observer.trigger()

    expect(disconnect).not.toHaveBeenCalled()
    expect(observe.mock.calls).toEqual([[added]])
    expect(observer.targets).toEqual(new Set([root, surface, added]))

    observe.mockClear()
    document.body.insertBefore(added, surface)
    observer.trigger()
    ...
    surface.remove()
    observer.trigger()
    expect(unobserve.mock.calls).toEqual([[surface]])
    expect(observer.targets).toEqual(new Set([root, added]))
    expect(disconnect).not.toHaveBeenCalled()
  ...
  it('registers current resize targets after renderer resources are released and restored', async () => {
    const root = document.createElement('div')
    const canvas = document.createElement('canvas')
    root.append(canvas)
    document.body.append(root)
    const removed = appendOpticalSurface('app-hover-lift-card', { height: 220, width: 320, x: 40, y: 80 })
    const retained = appendOpticalSurface('app-hover-lift-card', { height: 180, width: 280, x: 400, y: 80 })
    const active = ref(true)
    const scope = effectScope()
    const renderer = scope.run(() =>
      useGlassOpticalRenderer({
    ...
    await vi.waitFor(() => expect(renderer?.state.value).toBe('ready'))
    const original = ResizeObserverMock.instances.find(instance => instance.targets.has(root))!
    active.value = false
    await nextTick()
    expect(original.targets.size).toBe(0)
    removed.remove()
    const added = appendOpticalSurface('app-hover-lift-card', { height: 180, width: 280, x: 700, y: 80 })
    active.value = true

    await vi.waitFor(() => {
      expect(renderer?.state.value).toBe('ready')
      expect(ResizeObserverMock.instances.some(instance => instance !== original && instance.targets.has(root))).toBe(
        true,
      )
    })
    const restored = ResizeObserverMock.instances.find(instance => instance !== original && instance.targets.has(root))
    expect(restored?.targets).toEqual(new Set([root, retained, added]))
    scope.stop()
    expect(restored?.targets.size).toBe(0)
  })

  it.each(['attach', 'reparent', 'resume'] as const)(
    'tracks an empty scroll presentation root after %s without resetting the observer',
    async change => {
      const canvas = document.createElement('canvas')
      const originalRoot = document.createElement('div')
      const nextRoot = document.createElement('div')
      document.body.append(originalRoot, nextRoot)
      if (change !== 'attach') originalRoot.append(canvas)
      const scope = effectScope()
      const renderer = scope.run(() =>
        useGlassOpticalRenderer({
          active: ref(true),
      ...
      nextRoot.append(canvas)
      if (change === 'resume') {
        observer.trigger()
        expect(observer.targets.has(nextRoot)).toBe(false)
        documentHasFocus = true
        window.dispatchEvent(new Event('focus'))
      } else window.dispatchEvent(new Event('resize'))

      await vi.waitFor(() => expect(observer.targets).toEqual(new Set([nextRoot])))
      expect(disconnect).not.toHaveBeenCalled()
      scope.stop()
      expect(observer.targets.size).toBe(0)
    },
  ...
  it('retains a presentation root when it stops qualifying as an optical surface', async () => {
    const root = appendOpticalSurface('app-hover-lift-card', { height: 220, width: 320, x: 40, y: 80 })
    const canvas = document.createElement('canvas')
    root.append(canvas)
    const scope = effectScope()
    const renderer = scope.run(() =>
    ...
    const unobserve = vi.spyOn(observer, 'unobserve')
    const observe = vi.spyOn(observer, 'observe')
    root.dataset.glassOpticalMode = 'excluded'
    observer.trigger()
    window.dispatchEvent(new Event('resize'))

    await nextTick()
    expect(observer.targets).toEqual(new Set([root]))
    expect(unobserve).not.toHaveBeenCalled()
    expect(observe).not.toHaveBeenCalled()
    scope.stop()
  })
  ...
  it.each([180, 1000])('resumes ripple at %sms without duplicate steps or expired pixels', async resumeAt => {
    const three = await import('three')
    let visibilityState: DocumentVisibilityState = 'visible'
    let now = 0
    let interactionListener: ((event: PointerEvent | TouchEvent) => void) | null = null
    ...
    if (resumeAt === 180) {
      ...
      expect(uniforms.uHasRippleTexture.value).toBe(1)
      render.mockClear()
      const nextFrames = [...callbacks.values()]
      callbacks.clear()
      now = resumeAt + 1000 / 120
      const dynamicFrame = nextFrames.find(callback => callback.name === 'renderInteractionFrame')
      expect(dynamicFrame).toBeDefined()
      dynamicFrame!(now)
      expect(render).not.toHaveBeenCalled()
      const dueFrames = [...callbacks.values()]
      callbacks.clear()
      now = resumeAt + 1000 / 60
      dueFrames.forEach(callback => callback(now))
      expect(render).toHaveBeenCalled()
      scope.stop()
      surface.remove()
      return
  ...
  it('coalesces scroll updates and stops after two stable geometry frames without injecting motion', async () => {
    vi.spyOn(window, 'innerWidth', 'get').mockReturnValue(1200)
    vi.spyOn(window, 'innerHeight', 'get').mockReturnValue(800)
    let scrollY = 0
    vi.spyOn(window, 'scrollY', 'get').mockImplementation(() => scrollY)
  ...
  it.each(['off', 'fluid', 'ripple'] as const)(
    'keeps native ownership across sparse %s scroll input and restores once after the final intent',
    async dynamicsMode => {
      vi.spyOn(window, 'innerWidth', 'get').mockReturnValue(1200)
      vi.spyOn(window, 'innerHeight', 'get').mockReturnValue(800)
      let scrollY = 0
      ...
      vi.spyOn(window, 'cancelAnimationFrame').mockImplementation(id => callbacks.delete(id))
      const flushFrames = () => {
        for (let pass = 0; pass < 4 && callbacks.size > 0; pass++) {
          const scheduled = [...callbacks.values()]
          callbacks.clear()
          scheduled.forEach(callback => callback(performance.now() + pass * 8))
        }
      }
      appendOpticalSurface('app-hover-lift-card', { height: 300, width: 400, x: 40, y: 120 })
      const scope = effectScope()
      ...
      await vi.waitFor(() => expect(renderer?.state.value).toBe('ready'))
      flushFrames()
      const initialFrames = renderer!.renderedFrames.value
      vi.useFakeTimers({ toFake: ['setTimeout', 'clearTimeout'] })
      ...
      for (let input = 0; input < 5; input++) {
        ...
        expect(document.documentElement.dataset.glassScrollPresentation).toBe('native')
        expect(renderer!.renderedFrames.value).toBe(initialFrames)
        expect(callbacks.size).toBe(0)
      }

      // 到达滚动边界后仍可能有 wheel 意图而没有 scroll 事件，计时器也必须完成交接。
      window.dispatchEvent(new WheelEvent('wheel', { deltaY: 80 }))
      vi.advanceTimersByTime(179)
      expect(renderer!.renderedFrames.value).toBe(initialFrames)
      vi.advanceTimersByTime(1)
      expect(renderer!.renderedFrames.value).toBe(initialFrames + 1)
      expect(document.documentElement).not.toHaveAttribute('data-glass-scroll-presentation')
  ...
  it.each(['fluid', 'ripple'] as const)(
    'restores %s on a moving pointer only after scroll geometry settles',
    async dynamicsMode => {
      vi.spyOn(window, 'innerWidth', 'get').mockReturnValue(1200)
      vi.spyOn(window, 'innerHeight', 'get').mockReturnValue(800)
      const callbacks = new Map<number, FrameRequestCallback>()
      ...
      window.dispatchEvent(new WheelEvent('wheel', { deltaY: 80 }))
      window.dispatchEvent(new Event('scroll'))
      window.dispatchEvent(new MouseEvent('pointermove', { clientX: 880, clientY: 700 }))
      expect(document.documentElement.dataset.glassScrollPresentation).toBe('native')
      flushFrames()

      const anchorX = dynamicsMode === 'fluid' ? 880 : 900
      window.dispatchEvent(new MouseEvent('pointermove', { clientX: anchorX, clientY: 700 }))
      expect(document.documentElement.dataset.glassScrollPresentation).toBe('native')

      // 新的滚轮意图先撤销稳定判断，即便 scroll 事件还未派发也不能揭示旧坐标。
      window.dispatchEvent(new WheelEvent('wheel', { deltaY: 80 }))
      window.dispatchEvent(new MouseEvent('pointermove', { clientX: 850, clientY: 700 }))
      expect(document.documentElement.dataset.glassScrollPresentation).toBe('native')
      window.dispatchEvent(new Event('scroll'))
      flushFrames()

      const beforeInteraction = renderer!.renderedFrames.value
      window.dispatchEvent(new MouseEvent('pointermove', { clientX: 160, clientY: 180 }))
      expect(document.documentElement).not.toHaveAttribute('data-glass-scroll-presentation')
      expect(renderer!.renderedFrames.value).toBe(beforeInteraction + 1)
      flushFrames()
      expect(renderer!.renderedFrames.value).toBeGreaterThan(beforeInteraction + 1)
      scope.stop()
    },
  )

  it.each(['uncommitted-intent', 'nested-scroll', 'pending-stability'] as const)(
    'does not reveal native presentation on pointer input during %s',
    async boundary => {
      vi.spyOn(window, 'innerWidth', 'get').mockReturnValue(1200)
      vi.spyOn(window, 'innerHeight', 'get').mockReturnValue(800)
      const callbacks = new Map<number, FrameRequestCallback>()
      ...
      vi.spyOn(window, 'cancelAnimationFrame').mockImplementation(id => callbacks.delete(id))
      const flushFrames = (limit = 4) => {
        for (let pass = 0; pass < limit && callbacks.size > 0; pass++) {
          const scheduled = [...callbacks.values()]
          callbacks.clear()
          scheduled.forEach(callback => callback(performance.now() + pass * 8))
        }
      ...
      try {
        ...
        if (boundary === 'uncommitted-intent') {
          flushFrames(1)
          expect(callbacks.size).toBe(1)
          window.dispatchEvent(new WheelEvent('wheel', { deltaY: 80 }))
        }
  ...
  it.each(
    ...
    ).flatMap(([quality, dynamicsMode]) =>
      [60, 75, 90, 120, 144].map(refreshRate => ({ quality, dynamicsMode, refreshRate })),
    ),
  )(
    'budgets $quality $dynamicsMode rendering on a $refreshRate Hz display',
    async ({ quality, dynamicsMode, refreshRate }) => {
      vi.spyOn(window, 'innerWidth', 'get').mockReturnValue(1200)
      vi.spyOn(window, 'innerHeight', 'get').mockReturnValue(800)
      const three = await import('three')
      const render = vi.spyOn(three.WebGLRenderer.prototype, 'render')
      ...
      for (let frame = 1; frame <= refreshRate; frame += 1) {
        const timestamp = start + 100 + 1000 / 60 + (frame * 1000) / refreshRate
        const pointer = new MouseEvent('pointermove', { clientX: 160 + (frame % 20), clientY: 180 })
        Object.defineProperty(pointer, 'timeStamp', { value: timestamp })
        window.dispatchEvent(pointer)
        render.mockClear()
        tick(timestamp)
        if (render.mock.calls.length > 0) renderedTicks += 1
      }
      expect(renderedTicks).toBeGreaterThanOrEqual(quality === 'balanced' ? 59 : refreshRate)
      expect(renderedTicks).toBeLessThanOrEqual(quality === 'balanced' ? 61 : refreshRate)
      scope.stop()
    },
  )
